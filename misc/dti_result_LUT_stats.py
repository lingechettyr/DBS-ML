...
num_tracts = len(preData[str(.06)])

thresholds_60 = [] #np.empty(num_tracts)
for i in range(num_tracts):
    thresholds_60.append(preData[str(.06)][str(i)])
    #thresholds[i] = preData[str(pulse_width)][str(i)]

thresholds_500 = [] #np.empty(num_tracts)
for i in range(num_tracts):
    thresholds_500.append(preData[str(.5)][str(i)])

thresholds_all = []
for pwd in pulse_widths_all:
    for i in range(num_tracts):
        thresholds_all.append(preData[str(pwd/1000)][str(i)])
