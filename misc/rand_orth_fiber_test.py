...
for tract_index in range(len(xCompPos)):
    if lines == 1:
        ...
        temp_pos_tuple = (yCompPos[tract_index], zCompPos[tract_index])
        if temp_pos_tuple in val_pos:
            mlab.points3d(xCompPos[tract_index], yCompPos[tract_index],zCompPos[tract_index], color = (1,0,0), scale_factor=.2, opacity=1)
        else:
...
result_dict = {}
val_pos_out = []#tuple[k*1000 for k in val_pos]
print(val_pos)
for i in range(len(val_pos)):
    val_pos_out.append((int((round(val_pos[i][0],3)-lead_radius)*1000), int((round(val_pos[i][1],3)-contact_vertical_center)*1000)))

result_dict["val_pos"] = val_pos_out
